...
#import "HPAlipayReCertifyVC.h"
//导入芝麻信用
#import <ZMCreditSDK/ALCreditService.h>

@interface HPAlipayReCertifyVC ()
...
@implementation HPAlipayReCertifyVC
...
- (void)viewDidLoad {
    ...
    _phoneTextField.text = loginData.phone;
...
-(void)configCodeBtn
{
    ...
    UIImage *highlightImage = [HPUtile imageFromColor:[UIColor colorWithWhite:1.000 alpha:0.700] andSize:self.codeButton.size];
    UIImage *disableImage = [HPUtile imageFromColor:[UIColor colorWithWhite:0.500 alpha:0.500] andSize:self.codeButton.size];
    //三种状态
    ...
    RAC(_codeButton,enabled) =[RACSignal combineLatest:@[_nameTextField.rac_textSignal, _cardTextField.rac_textSignal] reduce:^id(NSString *name, NSString * cardNumber){
        return @(name.length >= 2 && cardNumber.length == 18);
    }];
}
...
- (IBAction)codeButtonClick:(UIButton *)sender
{
    ...
    if (!isMatch) {
        ...
        NSMutableDictionary *params = [HPUtile getBasicDictionary];
        //标明注册才能发送验证码 让注册
...
- (void)result:(NSMutableDictionary*)dic{
    ...
    [[HPHttpManager shareManager] postReqWithBaseUrlStr:kBaseURL surfixUrlStr:@"zmxy/submit.json" params:params success:^(NSDictionary *responseObject) {
        ...
        if (code == 0) {//授权成功
            ...
            [HPUtile storeObjects:@[  responseObject[@"id"],
                                      loginData.license,
                                      loginDataBinary
                                      ]
                         withKeys:@[  kUserID,
            ...
            //发送一个通知给四个主界面登录成功
            [[NSNotificationCenter defaultCenter] postNotificationName:kdidDismissNotification object:nil];
